...
class Tracelib {
    ...
    getFT(from,to){
        let sum=0,cnt=0;
        console.log(this._timelineLoader.performanceModel.frames());
        this._timelineLoader.performanceModel.frames().forEach(({ duration, startTime }) => {
...
let JANK_TRACE_LOG = require('./results/ar/three/ar-three-gltf-Box-64.json');
const tasks = new Tracelib(JANK_TRACE_LOG);
let clickScriptTimes = tasks.getClickTime();
let click = 3168.3;
let firstRenderTime = 3368.2;
let firstRenderScript = clickScriptTimes.end + firstRenderTime - click;
let from = firstRenderScript;
let to = from + 60000;
tasks.getFT(from,to);
